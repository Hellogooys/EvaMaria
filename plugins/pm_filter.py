...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("⚠️ 𝐁𝐫𝐨 𝐬𝐞𝐚𝐫𝐜𝐡 𝐲𝐨𝐮𝐫 𝐨𝐰𝐧 𝐟𝐢𝐥𝐞 . 𝐃𝐨𝐧'𝐭 𝐫𝐞𝐪𝐮𝐞𝐬𝐭 𝐨𝐭𝐡𝐞𝐫𝐬 𝐟𝐢𝐥𝐞 😩", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if (st.status == "creator") or (str(userid) in ADMINS):
            ...
            await query.answer("⚠︎ 𝚈𝙾𝚄 𝙽𝙴𝙴𝙳 𝚃𝙾 𝙱𝙴 𝙶𝚁𝙾𝚄𝙿 𝙰𝙳𝙼𝙸𝙽 𝙾𝚁 𝙾𝚆𝙽𝙴𝚁 ⚠︎", show_alert=True)
        ...
        if mkact:
            ...
            await query.message.edit_text('⚠︎ 𝚂𝙾𝙼𝙴 𝙴𝚁𝚁𝙾𝚁 𝙾𝙲𝙲𝚄𝚁𝙴𝙳 ⚠︎', parse_mode="md")
        return await query.answer('Piracy Is Crime')
    ...
    if query.data.startswith("file"):
        ...
        if not files_:
            return await query.answer('𝙽𝙾 𝚂𝚄𝙲𝙷 𝙵𝙸𝙻𝙴 𝙴𝚇𝙸𝚂𝚃.')
        files = files_[0]
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("𝙸 𝙻𝙸𝙺𝙴 𝚈𝙾𝚄𝚁 𝚂𝙼𝙰𝚁𝚃𝙽𝙴𝚂 𝙱𝚄𝚃 𝙳𝙾𝙽𝚃 𝙱𝙴 𝙾𝚅𝙴𝚁 𝚂𝙼𝙰𝚁𝚃 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
...
async def advantage_spell_chok(msg):
    ...
    if gs_parsed:
        for mov in gs_parsed:
            imdb_s = await get_poster(mov.strip(), bulk=True)  # searching eachasync def advantage_spell_chok(msg):
    query = re.sub(
        r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|new|latest|br((o|u)h?)*|^h(e|a)?(l)*(o)*|mal(ayalam)?|t(h)?amil|file|that|find|und(o)*|kit(t(i|y)?)?o(w)?|thar(u)?(o)*w?|kittum(o)*|aya(k)*(um(o)*)?|full\smovie|any(one)|with\ssubtitle(s)?)",
        "", msg.text, flags=re.IGNORECASE)  # plis contribute some common words
    query = query.strip() + " movie"
    ...
    if not movielist: keyword in imdb
            if imdb_s:
                movielist += [movie.get('title') for movie in imdb_s]
    movielist += [(re.sub(r'(\-|\(|\)|_)', '', i, flags=re.IGNORECASE)).strip() for i in gs_parsed]
    movielist = list(dict.fromkeys(movielist))  # removing duplicates
